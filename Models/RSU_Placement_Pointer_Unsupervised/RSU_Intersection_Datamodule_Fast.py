...
class RSU_Intersection_Dataset(Dataset):
    def __init__(self,
        ...
        self.knapsack_algorithm = knapsack_algorithm
    ...
    def create_scenarios(self):
        ...
        self.scenarios_best_packs = -np.ones(shape=(self.scenarios.shape[0], self.max_budget))
        ...
        for i in range(self.n_scenarios):
            for j in range(self.budgets.shape[0]):
                ...
                best_reward, best_weight, best_pack = self.calculate_best_reward_knapsack(self.scenarios[index, :len(self.scenario_idx[index,self.scenario_idx[index]>-1])], self.budgets[j])
                self.scenarios_best_packs[index,:self.budgets[j]] = best_pack
                self.scenarios_best_rewards[index] = best_reward
                index += 1

    def calculate_best_reward_knapsack(self,intersections, budget):
        return self.knapsack_algorithm.ka_algorithm(torch.tensor(intersections[None,:,:]), torch.tensor(budget))
